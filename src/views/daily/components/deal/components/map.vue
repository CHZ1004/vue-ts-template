...
import { DailyParams, DailyProvince, getTradeProvinceDistribution } from '@/api';
...
const { loading, onSuccess, send } = useRequest(() => getTradeProvinceDistribution(props.model));
onSuccess(({ data }) => {
  Array.isArray(data) &&
    data.map((item, index) => {
